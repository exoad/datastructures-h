...
/**
 * @brief Inserts a node in the middle of a LinkedList
 * @param theRest the rest of the linkedlist
 * @param value the value of the node
 * @param pos where to insert it
 * @return the completed list
 */
ListNode* insertMiddle(ListNode* theRest,string value,int pos)
{
    ListNode* curr=new ListNode(value,nullptr);
    if(pos<=0)
        insertFirst(theRest,value);
    ListNode* curr=theRest;
    int i=0;
    ...
    if(curr==nullptr)
        return theRest;
    ListNode* res=new ListNode(value,curr->getNext());
    curr->next=res;
    return theRest;
}

/**
 * @brief Removes a node in the middle of a LinkedList
 * @param theRest the rest of the linkedlist
 * @param pos where to insert it
 * @return the completed list
 */
ListNode* remove(ListNode* theRest,int pos)
{
    if(pos<=0)
    {
        ListNode* res=theRest->getNext();
        delete theRest;
        return res;
    }
    ListNode* curr=theRest;
